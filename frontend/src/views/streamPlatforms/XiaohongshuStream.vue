<template>
  <div class="xiaohongshu-bg-container">
    <div class="bg-decoration">
      <div class="floating-shape shape-1"></div>
      <div class="floating-shape shape-2"></div>
      <div class="floating-shape shape-3"></div>
      <div class="floating-shape shape-4"></div>
    </div>
    <div class="xiaohongshu-guide">
      <div class="header">
        <div class="logo-title-row">
          <span class="xiaohongshu-logo">
            <svg width="48" height="21" viewBox="0 0 72 32" fill="none" xmlns="http://www.w3.org/2000/svg">
              <g clip-path="url(#clip0_243_289)">
                <path d="M56 0H16C7.16344 0 0 7.16344 0 16C0 24.8366 7.16344 32 16 32H56C64.8366 32 72 24.8366 72 16C72 7.16344 64.8366 0 56 0Z" fill="#FF2442"/>
                <path fill-rule="evenodd" clip-rule="evenodd" d="M55.8066 12.0417C55.8084 12.0617 55.8221 12.0739 55.8414 12.0754C55.8726 12.0778 55.9038 12.0784 55.9352 12.0784C56.1676 12.0787 56.4001 12.0785 56.6325 12.0785C56.8517 12.0785 57.0708 12.0779 57.2899 12.0788C57.3978 12.0794 57.5044 12.0665 57.6091 12.0433C58.4192 11.864 58.9241 11.0754 58.747 10.2645C58.5892 9.54337 57.906 9.0042 57.0956 9.11147C56.3484 9.21016 55.809 9.835 55.8035 10.5926C55.8004 11.0489 55.8031 11.5052 55.8033 11.9615C55.8033 11.9881 55.8042 12.0151 55.8066 12.0417ZM34.3317 21.0146C33.9533 21.016 31.807 21.0219 31.1867 21.0185C30.72 21.0158 30.2071 20.9356 29.7581 20.785C29.735 20.7773 29.7101 20.7881 29.7001 20.8103L28.3654 23.7004C28.3554 23.7229 28.3641 23.7493 28.3857 23.7611C28.5924 23.8729 29.0118 23.9924 29.309 23.9924C29.5325 23.991 29.5153 23.9993 29.7389 23.999C30.6094 23.9977 32.7506 23.9929 33.0048 23.9924C33.0236 23.9924 33.04 23.9813 33.0476 23.9644L34.3751 21.0806C34.3889 21.0496 34.366 21.0145 34.3317 21.0146ZM17.6539 12.0979C17.6271 12.0969 17.6004 12.0975 17.5735 12.0975C16.6792 12.0975 15.7849 12.0973 14.8908 12.0979C14.8578 12.0979 14.7985 12.0973 14.7668 12.1024C14.7518 12.1047 14.749 12.112 14.7471 12.1166C14.7432 12.1266 14.735 12.2126 14.731 12.2654C14.7086 12.5641 14.6879 12.863 14.6651 13.1616C14.6443 13.4337 14.6205 13.7054 14.5991 13.9773C14.5768 14.2626 14.556 14.548 14.5338 14.8333C14.5113 15.1276 14.4876 15.4217 14.465 15.716C14.4423 16.0147 14.4208 16.3135 14.3979 16.6122C14.3766 16.8886 14.3539 17.1648 14.3323 17.4414C14.3184 17.6197 14.3014 17.7978 14.2925 17.9764C14.2704 18.4241 14.2288 18.7017 14.0409 19.1085C13.9922 19.2142 13.979 19.1864 14.046 19.3184C14.1326 19.4893 15.3183 22.1585 15.5155 22.5628C15.5298 22.5922 15.536 22.6008 15.5463 22.6008C15.5613 22.6008 15.6079 22.5539 15.6347 22.5198C15.9093 22.1701 16.1346 21.7898 16.327 21.3897C16.6071 20.8075 16.8138 20.1989 16.9728 19.5736C17.0875 19.1227 17.1768 18.6665 17.2346 18.2047C17.2601 18.0007 17.2736 17.7953 17.2898 17.5902C17.3284 17.1045 17.3648 16.6184 17.4027 16.1325C17.4362 15.7046 17.4709 15.2769 17.5041 14.849C17.5281 14.5369 17.5495 14.2246 17.5738 13.9125C17.6048 13.5114 17.6381 13.1104 17.6699 12.7091C17.6845 12.5264 17.6981 12.3437 17.7123 12.1609C17.7163 12.1084 17.709 12.1001 17.6539 12.0979ZM28.3297 19.1085C28.142 18.7017 28.1003 18.4241 28.0781 17.9764C28.0692 17.7978 28.0524 17.6197 28.0385 17.4414C28.0168 17.1648 27.994 16.8886 27.9728 16.6122C27.9499 16.3135 27.9285 16.0147 27.9056 15.716C27.8832 15.4217 27.8596 15.1276 27.8368 14.8333C27.8148 14.548 27.7939 14.2626 27.7716 13.9773C27.7502 13.7054 27.7266 13.4337 27.7056 13.1616C27.6827 12.863 27.6622 12.5641 27.6396 12.2654C27.6356 12.2126 27.6303 12.1353 27.6219 12.1124C27.6136 12.0921 27.5239 12.0979 27.4799 12.0979C26.5858 12.0973 25.6915 12.0975 24.7972 12.0975C24.7705 12.0975 24.7436 12.0969 24.7168 12.0979C24.6617 12.1001 24.6544 12.1084 24.6586 12.1609C24.6725 12.3437 24.6862 12.5264 24.7008 12.7091C24.7327 13.1104 24.7658 13.5114 24.7969 13.9125C24.8213 14.2246 24.8425 14.5369 24.8666 14.849C24.8997 15.2769 24.9347 15.7046 24.9679 16.1325C25.006 16.6184 25.0424 17.1045 25.0809 17.5902C25.0971 17.7953 25.1106 18.0007 25.1362 18.2047C25.1938 18.6665 25.2831 19.1227 25.3978 19.5736C25.557 20.1989 25.7635 20.8075 26.0437 21.3897C26.2362 21.7898 26.4615 22.1701 26.7361 22.5198C26.7629 22.5539 26.8042 22.6153 26.8253 22.6111C26.8364 22.6089 26.8382 22.5972 26.8488 22.5758C27.0252 22.2199 28.2371 19.4914 28.3248 19.3184C28.3917 19.1864 28.3785 19.2142 28.3297 19.1085ZM35.5355 17.4047H33.9077C33.7365 17.4047 33.6221 17.2292 33.6918 17.0734L35.8887 12.1751C35.9027 12.1441 35.8798 12.109 35.8456 12.109L33.3872 12.1038C33.1858 12.1034 33.0518 11.8965 33.1344 11.7137L34.7698 8.06642C34.7837 8.0352 34.761 8.00013 34.7267 8.00013C34.3507 7.99998 32.6198 7.99984 31.8142 8.00043C31.7955 8.00043 31.7792 8.01138 31.7714 8.02824L30.0432 11.8874C29.8674 12.2392 29.6985 12.5945 29.546 12.9571C29.4525 13.1798 29.3649 13.4044 29.2989 13.6372C29.2656 13.7546 29.2513 13.8724 29.2613 13.9946C29.2861 14.3019 29.4258 14.5373 29.6891 14.6986C29.7656 14.7454 29.8463 14.7837 29.931 14.813C30.1311 14.8824 30.3364 14.9207 30.5486 14.9206C30.9912 14.9203 31.2066 14.9203 31.6492 14.9207C31.6927 14.9209 30.5589 17.4038 30.3547 17.8857C30.2553 18.1199 30.1621 18.3562 30.092 18.6014C30.062 18.7059 30.0456 18.8143 30.051 18.9228C30.0672 19.2493 30.2071 19.4976 30.4801 19.6646C30.5605 19.7139 30.6454 19.7533 30.7345 19.7836C30.9393 19.8537 31.1494 19.8881 31.3662 19.8878C32.2915 19.8867 33.4535 19.8873 34.3789 19.8873C34.3945 19.8873 34.4332 19.8866 34.4627 19.8867C34.4815 19.8867 34.4984 19.8759 34.5061 19.8589L35.5786 17.471C35.5926 17.4397 35.5697 17.4047 35.5355 17.4047ZM22.6012 8.19455L19.7843 8.19588C19.7509 8.19588 19.7237 8.22237 19.7237 8.25581C19.7216 8.96528 19.7025 15.8091 19.7025 19.2628V20.9997C19.7025 21.5355 19.2024 21.4875 19.18 21.4875H17.8946C17.8634 21.4875 17.832 21.4869 17.8007 21.4877C17.7252 21.4899 17.7191 21.4948 17.7243 21.576C17.7286 21.6444 18.5933 23.56 18.7612 23.9314C18.7785 23.9697 18.8156 23.9938 18.8577 23.9944C19.1495 23.9983 20.2888 24.0101 20.6077 23.9684C20.8933 23.9312 21.1682 23.8558 21.4247 23.7216C21.9827 23.4297 22.3405 22.9395 22.5352 22.2945C22.6325 21.9721 22.6743 21.6356 22.6747 21.299C22.677 19.1611 22.6756 17.0232 22.6756 14.8852C22.6755 12.842 22.6636 8.79306 22.6621 8.25433C22.662 8.22089 22.6346 8.19455 22.6012 8.19455ZM44.5913 21.0626C44.59 21.0356 44.5726 21.0226 44.5479 21.0216C44.5078 21.02 44.4675 21.0204 44.4273 21.0204H41.9063C41.8749 21.0204 41.8437 21.0208 41.8124 21.0203C41.7232 21.0185 41.7229 21.018 41.721 20.9303C41.7205 20.9035 41.721 20.8766 41.721 20.8498V12.2342C41.721 12.0995 41.7116 12.1022 41.8522 12.1031C41.8789 12.1034 42.92 12.1031 43.4611 12.1032C43.6086 12.1032 43.6005 12.1121 43.6005 11.9665C43.6005 11.0675 43.6006 10.1682 43.6005 9.26919C43.6005 9.11561 43.6131 9.12774 43.4532 9.12774C41.2443 9.12774 39.1301 9.12774 36.9213 9.12774C36.8011 9.12774 36.7818 9.11576 36.7818 9.26357C36.7818 10.1627 36.7818 11.0619 36.7819 11.961C36.7819 12.1143 36.7698 12.1032 36.9291 12.1032C37.4924 12.1032 37.9612 12.1031 38.5246 12.1032C38.6643 12.1032 38.6548 12.0966 38.6556 12.2342V12.261C38.6556 15.1284 38.6556 17.9958 38.6555 20.8631C38.6555 21.0392 38.669 21.0201 38.4962 21.0201C37.8051 21.0206 36.4265 21.0154 36.0624 21.0207C35.9725 21.0219 35.9507 21.0765 35.9507 21.0765L34.6495 23.9093C34.6495 23.9093 34.6234 23.9571 34.6351 23.9767C34.6469 23.9962 34.6596 23.9922 34.7435 23.9923C35.2205 23.9931 36.8924 23.9959 37.9002 23.9959H44.4057C44.446 23.9959 44.4863 23.9968 44.5264 23.9955C44.5811 23.9934 44.5903 23.9845 44.5928 23.9342C44.5939 23.9119 44.5934 23.8895 44.5934 23.8672C44.5934 22.9591 44.5934 22.051 44.5934 21.1429C44.5934 21.116 44.5928 21.0892 44.5913 21.0626ZM52.3861 14.6622C52.3847 14.6869 52.3725 14.7047 52.3462 14.7072C52.3196 14.7097 52.2928 14.7112 52.2661 14.7112C51.833 14.7113 50.9141 14.7153 50.4809 14.7152C50.4586 14.7152 50.4364 14.7145 50.4142 14.7128C50.3882 14.7109 50.3735 14.6949 50.372 14.6699C50.3699 14.6344 50.3704 14.5986 50.3704 14.5629L50.3534 12.2377C50.3514 12.1038 50.3435 12.1059 50.483 12.1062C50.7777 12.1069 51.5583 12.1024 51.8531 12.1024C51.929 12.1025 52.0039 12.1062 52.0778 12.1303C52.2616 12.1907 52.3673 12.3196 52.3848 12.5113C52.3876 12.5424 52.387 12.5738 52.387 12.605C52.3872 13.2169 52.3872 13.9701 52.3872 14.5819C52.3872 14.6087 52.3875 14.6356 52.3861 14.6622ZM58.7196 16.9824C58.5615 16.1491 58.1211 15.5194 57.3686 15.1181C56.8537 14.8436 56.3001 14.7176 55.7195 14.7032C55.6078 14.7004 55.4959 14.7035 55.3841 14.7022C55.3109 14.7013 55.3084 14.6976 55.306 14.627C55.3054 14.6045 55.3058 14.5822 55.3058 14.5598C55.3058 13.9738 55.3055 13.2465 55.306 12.6606C55.3061 12.4143 55.2935 12.1688 55.2598 11.9247C55.2122 11.5815 55.122 11.2507 54.9743 10.9366C54.7006 10.3536 54.2738 9.91519 53.71 9.61024C53.129 9.29612 52.5044 9.14313 51.8458 9.12922C51.5596 9.12316 50.7875 9.131 50.5011 9.13055C50.3719 9.13041 50.3739 9.13706 50.3741 9.00878C50.3745 8.79394 50.3744 8.3474 50.3742 8.08906C50.3742 8.06213 50.3702 8.04304 50.3545 8.02721C50.3393 8.01227 50.2894 8.01508 50.2894 8.01508L47.4551 8.01596C47.4551 8.01596 47.4247 8.01448 47.4119 8.02824C47.4003 8.04053 47.3963 8.05147 47.3963 8.07145C47.3962 8.31854 47.3963 8.77175 47.3962 8.98289C47.3962 9.0187 47.396 9.0545 47.3941 9.09016C47.3926 9.11635 47.3758 9.12937 47.3505 9.13026C47.3147 9.13144 47.2791 9.13159 47.2432 9.13159C46.6888 9.13174 46.1344 9.13174 45.5798 9.13174C45.5441 9.13174 45.5083 9.131 45.4725 9.13248C45.4311 9.1344 45.4201 9.14639 45.418 9.19092C45.4165 9.22214 45.4172 9.25351 45.4172 9.28488C45.4172 10.1752 45.4172 11.0653 45.4172 11.9554C45.4171 12.1235 45.4094 12.1068 45.5696 12.1069C46.1375 12.1072 46.7053 12.1069 47.2733 12.1071C47.4268 12.1071 47.4074 12.3456 47.4074 12.3456L47.4058 14.3586L47.4057 14.4878C47.4057 14.4878 47.4129 14.7065 47.2933 14.7065C47.2473 14.7065 47.1526 14.7065 47.1064 14.7065C46.2585 14.7069 45.3742 14.7068 44.5304 14.7066C44.4039 14.7066 44.4184 14.8603 44.4184 14.8603V15.0312V17.4165V17.5563C44.4184 17.5563 44.3987 17.683 44.5374 17.683C45.4227 17.6837 46.3705 17.6833 47.2559 17.6834C47.4094 17.6834 47.3981 17.6715 47.3982 17.8304V19.0784C47.3982 20.7158 47.3981 22.2116 47.3982 23.849C47.3982 24.0128 47.3847 23.9997 47.5505 23.9997C48.4404 23.9999 49.33 23.9999 50.22 23.9997C50.3864 23.9997 50.372 24.0129 50.3722 23.849C50.3722 22.8246 50.3722 21.8003 50.3722 20.7757V17.8573C50.3722 17.8215 50.3713 17.7855 50.3726 17.7499C50.3744 17.6962 50.3838 17.6861 50.4338 17.684C50.4695 17.6826 50.5052 17.6834 50.5411 17.6834C51.1224 17.6833 52.1897 17.6794 52.7709 17.6794C53.437 17.6794 54.1035 17.6796 54.7695 17.6794C54.9511 17.6794 55.1254 17.7154 55.2892 17.7931C55.6093 17.9446 55.7751 18.2031 55.8019 18.553C55.8084 18.6376 55.8078 18.7229 55.8078 18.8078C55.8081 19.6802 55.8083 19.8921 55.8078 20.7644C55.8078 21.2469 55.6078 21.4492 55.1926 21.4492C54.3162 21.4498 53.9161 21.4495 53.0397 21.4495C52.9994 21.4495 52.9591 21.4488 52.9189 21.4501C52.8768 21.4517 52.8661 21.4633 52.8676 21.5091C52.8685 21.5494 53.7417 23.5303 53.9197 23.9224C53.9228 23.9293 53.9256 23.9357 53.9283 23.9416C53.9417 23.971 53.9707 23.99 54.0029 23.9906C54.013 23.9909 54.0243 23.991 54.0368 23.9913C54.2052 23.9943 54.5921 23.9999 54.9575 23.9955C55.2355 23.9921 55.9219 24.0029 56.1991 23.9774C56.4579 23.9537 56.7134 23.9122 56.962 23.8364C58.0613 23.5019 58.7789 22.5352 58.7797 21.3866C58.7805 20.1072 58.7792 18.9691 58.7804 17.6895C58.7805 17.4517 58.7639 17.216 58.7196 16.9824Z" fill="white"/>
              </g>
              <defs>
                <clipPath id="clip0_243_289">
                  <rect width="72" height="32" fill="white"/>
                </clipPath>
              </defs>
            </svg>
          </span>
          <h1>小红书直播推流码获取教程</h1>
        </div>
      </div>

      <div class="container">
        <h2>一、准备工作</h2>
        <div class="step-card">
          <div class="step-index">1</div>
          <div class="step-content">
            <h3>账号要求</h3>
            <p>获取小红书直播推流码需要满足以下条件：</p>
            <ul>
              <li>已完成小红书实名认证</li>
              <li>粉丝数≥1000或获得官方直播邀请</li>
              <li>账号无违规记录</li>
            </ul>
          </div>
        </div>
        <div class="warning">
          <strong>⚠️ 重要提示：</strong> 小红书推流码为一次性使用，每次直播需要重新获取。
        </div>
      </div>

      <div class="container">
        <h2>二、获取推流码详细步骤</h2>
        <div
          class="step-card"
          v-for="(step, index) in steps"
          :key="index"
        >
          <div class="step-index">{{ index + 2 }}</div>
          <div class="step-content">
            <h3>{{ step.title }}</h3>
            <p v-html="step.description"></p>
            <div v-if="step.image" class="screenshot-wrap">
              <img :src="step.image" :alt="step.title" class="screenshot" />
            </div>
            <div v-if="step.code" class="code-block">
              <span class="code-icon">💻</span>
              <span v-html="step.code"></span>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: 'XiaohongshuStreamGuide',
  data() {
    return {
      steps: [
        {
          title: '步骤1：开通直播权限',
          description: '打开小红书APP → 点击"我" → 进入"创作者中心" → 选择"更多服务" → 点击"直播" → 完成权限申请'
        },
        {
          title: '步骤2：下载直播助手',
          description: '访问小红书官网下载"小红书直播助手"PC端软件，安装并登录你的小红书账号。'
        },
        {
          title: '步骤3：获取推流码',
          description: `
            <ol>
              <li>打开小红书直播助手</li>
              <li>点击"创建直播" → 选择"专业开播"</li>
              <li>找到"第三方推流"选项</li>
              <li>系统将生成RTMP地址和流密钥</li>
            </ol>
          `,
          code: `
            RTMP地址: rtmp://push.xiaohongshu.com/live/<br>
            流密钥: xhs_1234567890abcdef<br>
            完整推流地址: rtmp://push.xiaohongshu.com/live/xhs_1234567890abcdef
          `
        }
      ]
    }
  }
}
</script>

<style scoped>
.xiaohongshu-bg-container {
  min-height: 100vh;
  width: 100vw;
  margin: 0;
  padding: 0;
  position: relative;
  background: linear-gradient(120deg, #fff 0%, #ffe5ea 40%, #ffb3c6 100%);
  overflow-x: hidden;
}

.bg-decoration {
  position: fixed;
  top: 0;
  left: 0;
  width: 100vw;
  height: 100vh;
  pointer-events: none;
  z-index: 0;
}

.floating-shape {
  position: absolute;
  border-radius: 50%;
  background: rgba(255, 36, 66, 0.10);
  animation: float 6s ease-in-out infinite;
}

.shape-1 {
  width: 120px;
  height: 120px;
  top: 12%;
  left: 8%;
  animation-delay: 0s;
}

.shape-2 {
  width: 180px;
  height: 180px;
  top: 60%;
  right: 10%;
  animation-delay: 2s;
}

.shape-3 {
  width: 90px;
  height: 90px;
  bottom: 18%;
  left: 18%;
  animation-delay: 4s;
}

.shape-4 {
  width: 140px;
  height: 140px;
  top: 28%;
  right: 28%;
  animation-delay: 1s;
}

@keyframes float {
  0%, 100% { transform: translateY(0px) rotate(0deg); }
  50% { transform: translateY(-20px) rotate(180deg); }
}

.xiaohongshu-guide {
  font-family: 'PingFang SC', 'Microsoft YaHei', sans-serif;
  line-height: 1.6;
  color: #333;
  max-width: 900px;
  margin: 0 auto;
  padding: 0 8px 40px 8px;
  min-height: 100vh;
  position: relative;
}

.xiaohongshu-guide::before {
  content: '';
  position: fixed;
  left: -120px;
  top: -120px;
  width: 320px;
  height: 320px;
  background: radial-gradient(circle, #ff2442 0%, #fff0 80%);
  opacity: 0.12;
  z-index: 0;
  pointer-events: none;
}

.xiaohongshu-guide::after {
  content: '';
  position: fixed;
  right: -100px;
  bottom: -100px;
  width: 260px;
  height: 260px;
  background: radial-gradient(circle, #ffb3c6 0%, #fff0 80%);
  opacity: 0.13;
  z-index: 0;
  pointer-events: none;
}

.header {
  display: flex;
  flex-direction: column;
  align-items: center;
  text-align: center;
  margin-bottom: 30px;
  padding: 32px 10px 24px 10px;
  background: linear-gradient(90deg, #ff2442 60%, #ff7a7a 100%);
  color: white;
  border-radius: 16px;
  box-shadow: 0 4px 24px #ff244233;
  position: relative;
  z-index: 1;
}

.header h1 {
  font-size: 2.2rem;
  font-weight: 800;
  margin: 0 0 8px 0;
  letter-spacing: 2px;
}

.header p {
  font-size: 1.1rem;
  color: #fffbe6;
  margin: 0;
}

.container {
  background-color: white;
  padding: 32px 20px 24px 20px;
  border-radius: 16px;
  box-shadow: 0 2px 12px rgba(255,36,66,0.08);
  margin-bottom: 32px;
}

h2 {
  color: #ff2442;
  border-bottom: 2px solid #ffcccc;
  padding-bottom: 8px;
  margin-top: 0;
  margin-bottom: 28px;
  font-size: 1.25rem;
  font-weight: 700;
  letter-spacing: 1px;
}

.step-card {
  display: flex;
  align-items: flex-start;
  background: #fff6f8;
  border-radius: 12px;
  box-shadow: 0 2px 8px #ff244211;
  margin-bottom: 28px;
  padding: 22px 18px 18px 18px;
  transition: box-shadow 0.2s;
  border-left: 6px solid #ff2442;
}

.step-card:hover {
  box-shadow: 0 6px 24px #ff244233;
}

.step-index {
  min-width: 38px;
  height: 38px;
  background: #ff2442;
  color: #fff;
  font-size: 1.3rem;
  font-weight: bold;
  border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
  margin-right: 18px;
  margin-top: 4px;
  box-shadow: 0 2px 8px #ff244233;
}

.step-content {
  flex: 1;
}

.step-content h3 {
  margin: 0 0 8px 0;
  font-size: 1.08rem;
  font-weight: 700;
  color: #ff2442;
}

.warning {
  background-color: #fff8e6;
  border-left: 4px solid #ffcc00;
  padding: 15px;
  margin: 20px 0;
  border-radius: 6px;
  font-size: 1rem;
}

.screenshot-wrap {
  background: #fff0f3;
  border-radius: 8px;
  padding: 12px;
  margin: 18px 0;
  text-align: center;
}

.screenshot {
  max-width: 100%;
  height: auto;
  border: 1px solid #ffd6de;
  border-radius: 6px;
  box-shadow: 0 2px 8px #ff244211;
}

.code-block {
  background-color: #f5f5f5;
  padding: 14px 16px;
  border-radius: 6px;
  font-family: 'JetBrains Mono', 'Menlo', 'Consolas', monospace;
  overflow-x: auto;
  margin-top: 12px;
  font-size: 1rem;
  color: #c7254e;
  position: relative;
  box-shadow: 0 2px 8px #ff244211;
}

.code-icon {
  position: absolute;
  left: 10px;
  top: 10px;
  font-size: 1.1rem;
  color: #ff2442;
}

@media (max-width: 600px) {
  .container {
    padding: 12px 4px 12px 4px;
    border-radius: 8px;
  }
  .step-card {
    flex-direction: column;
    align-items: stretch;
    padding: 14px 8px 10px 8px;
  }
  .step-index {
    margin: 0 0 10px 0;
    align-self: flex-start;
  }
}

.logo-title-row {
  display: flex;
  align-items: center;
  gap: 18px;
  justify-content: center;
}

.xiaohongshu-logo {
  display: flex;
  align-items: center;
  justify-content: center;
  width: 48px;
  height: 48px;
}
</style>